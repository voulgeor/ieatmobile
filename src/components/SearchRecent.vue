      ...
      <template v-if="hasData">
       ...
       <template v-for="(item,index) in data_recent" :key="item" >
        ...
        >
          ...
          <q-item-section side>
            <q-btn @click.stop="removeHistory(index)" round color="primary" icon="eva-close-outline" dense no-caps size="sm" flat  />
          </q-item-section>
        </q-item>
  ...
  methods: {
    ...
    removeHistory (index) {
      if (this.data_recent[index]) {
        ...
        APIinterface.setStorage('search_history', JSON.stringify(this.data_recent))
      }
    },
    toMenu (data) {
