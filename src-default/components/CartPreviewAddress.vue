   ...
   <q-item v-else>
      ...
      <q-item-section top>
        <q-item-label class="no-padding no-margin text-weight-bold">
          <q-chip  size="13px" color="white" icon="eva-home-outline" class="no-padding  no-margin transparent" >
      ...
      if (this.transaction_type === 'delivery') {
        ...
        APIinterface.checkoutAddress(placeId)
          .then(data => {
            ...
            if ((typeof this.mapsconfig !== 'undefined') && (this.mapsconfig !== null)) {
            ...
            this.map_center = { lat: parseFloat(data.details.data.latitude), lng: parseFloat(data.details.data.longitude) }
            this.map_marker = [
              { position: { lat: parseFloat(data.details.data.latitude), lng: parseFloat(data.details.data.longitude) }, icon: 'placeholder.png' }
            ]
          })
          .catch(error => {
        ...
        this.map_center = { lat: parseFloat(this.merchant_adddress.latitude), lng: parseFloat(this.merchant_adddress.lontitude) }
        this.map_marker = [
          { position: { lat: parseFloat(this.merchant_adddress.latitude), lng: parseFloat(this.merchant_adddress.lontitude) }, icon: 'placeholder.png' }
        ]
        if ((typeof this.mapsconfig !== 'undefined') && (this.mapsconfig !== null)) {
          this.maps_config = this.mapsconfig
          this.zoom = this.maps_config.zoom
