...
const APIinterface = {
  ...
  async getMerchantInfo(slug, place_id) {
    if (auth.authenticated()) {
      return api
        .get("/getMerchantInfo2?slug=" + slug + "&place_id=" + place_id, {
          headers: {
            "Content-Type": "application/json",
  ...
  async geStoreMenu(slug) {
    return api
      .post("/geStoreMenu", "slug=" + slug)
      .then((result) => {
        if (result.data.code === 1) {
          return result.data;
        } else {
  ...
  async getMenuItem(catId, itemUuid, slug) {
    if (auth.authenticated()) {
      return api
        .post(
          "/getMenuItem2",
  ...
  async Search(q, placeID) {
    return api
      .post("/Search", "q=" + q + "&place_id=" + placeID)
      .then((result) => {
        if (result.data.code === 1) {
          return result.data;
        } else {
